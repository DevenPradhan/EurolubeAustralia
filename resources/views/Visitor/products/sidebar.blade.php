<div
    class="space-y-3 w-max shrink-0 hidden sm:flex flex-col tracking-normal font-medium pt-16 px-12 h-full pb-60 text-[#ffffffc4] text-sm">
    <hr class=" opacity-30">
            ...
            <div class="space-y-3 text-xs flex flex-col">
                @foreach ($subCategories as $subCategory)
                    <a href="{{ route('searchCategory2', [
                        ...
                        class="ml-8 mr-4 py-1 hover:text-[#da4242] transition-colors 
                        {{ $url === $category->name . '/' . str_replace(['/', '-'], ' ', $subCategory->name) 
                        && str_contains($url, str_replace(['/', '-'], ' ', $subCategory->name)) 
                        ? 'text-[#da4242] font-semibold max-w-max' : '' }}">
                        {{ $subCategory->name }}
                    </a>
                    <hr class="opacity-30">

                    @if (isset($products) && str_contains($url, $category->name) && str_contains($url, $subCategory->name))
                        <div class="w-60 space-y-3 flex flex-col text-xs">
