<div class="container mx-auto flex flex-col space-y-20">
    ...
    <div class="flex flex-col space-y-5 mx-10">
                ...
                @foreach ($product->images as $image)
                    <div class="relative w-56 overflow-hidden h-40 group">
                        <img src="{{ asset('storage/images/' . $image->image_url) }}" alt=""
                            class="object-cover absolute inset-0 w-full h-full top-0 z-0 group-hover:brightness-75">
                        <div class="relative items-center justify-center z-10 hidden group-hover:flex my-16">
                            <x-danger-button class=""
                                onclick="return confirm('Are you sure you want to delete the image?\nYou will lose the image once it is deleted.')"
                                wire:click.prevent="deleteImage({{ $image->id }})" type="button">Delete
                            </x-danger-button>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="flex flex-col space-y-5 p-8">
            ...
            <p>Part No: &emsp;{{ isset($product->details) ? $product->details->part_no : '' }}</p>
            <p>Manual: &emsp;{{ isset($product->details) ? $product->details->manual : '' }}</p>
            <p>Dimensions: &emsp;{{ isset($product->features) ? $product->features->dimensions : '' }}</p>
            <p>Weight: &emsp;{{ isset($product->features) ? $product->features->weight : '' }}</p>
