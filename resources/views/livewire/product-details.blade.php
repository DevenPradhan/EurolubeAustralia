<div class="">
    <div class="p-8 inline-flex space-x-2">
        ...
        <button type="button" {{ Auth::user()->role == 'admin' ? '' : 'disabled' }}
            wire:click="changeValidity({{ $product }})"
            onclick="return confirm('Alter this item if you have confirmed with your colleagues') || event.stopImmediatePropagation()"
            class="px-3 py-2 border border-transparent uppercase text-xs tracking-widest font-semibold rounded-sm {{ $product->validity == 0 ? 'bg-gray-600' : 'bg-green-700' }} text-gray-200">
            {{ $product->validity == 0 ? 'Unverified' : 'Verified' }}
        </button>
        <form action="{{route('product.destroy', $product->id)}}" method="POST">
            @method('DELETE')
            @csrf
            <x-danger-button onclick="return confirm('Are you sure you want to remove this product?')" >
                Delete
             </x-danger-button>
        </form>
        ...
        @else
            @foreach ($product->images as $image)
                <div class="relative w-56 overflow-hidden h-40 group">
                    ...
                    <div
                        class="relative items-center justify-center z-10 hidden space-x-4 group-hover:flex my-16 transition-all">
                        <a href="{{ asset('storage/images/' . $image->image_url) }}" target="__blank" class="">
                            <x-eye />
                        </a>
                        <button class=""
