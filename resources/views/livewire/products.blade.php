        ...
        <div class="p-6">
            <form action="" wire:submit.prevent="addProduct">
                <div class="space-y-8" x-data="{ show_categories: @entangle('categoryDiv') }" x-init="@this.on('select-category', () => { show_categories = false })">
                    ...
                    <div class="flex p-3 rounded border h-60 bg-gray-300" x-show="show_categories"
                        x-transition.duration.100ms x-data="{ add_category: @entangle('activeDiv') }">
                            ...
                            <div class="flex h-10 rounded relative border w-full mt-5">
                                ...
                                <div class="">
                                    <input type="text" placeholder="Enter new Category"
                                        wire:model.debounce.500ms="newCategory"
                            ...
                            @if (!empty($newCategorySearch))
                                <div class="absolute top-25 flex w-full justify-center">
                                    <p class="text-sm text-red-800">{{ $newCategorySearch->name }} : <span
                                            class="font-semibold">This category has been taken</span></p>
                                </div>
                            @endif
                            <x-secondary-button class="mt-20"
                                wire:click.prevent="saveCategory">Save</x-secondary-button>
                        </div>
                    </div>
